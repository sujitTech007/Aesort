...
<div class="page-content">
    ...
    <form method="POST" action="{{ route('admin.subscription_plans.store') }}">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="from_sqft" class="form-control" value="{{ old('from_sqft') }}" required>
        </div>

        <div class="mb-3">
            <label>To SQFT</label>
            <input type="number" name="to_sqft" class="form-control" value="{{ old('to_sqft') }}" required>
        </div>
...
document.getElementById('add-feature').addEventListener('click', function() {
    ...
    div.innerHTML = `
        <input type="text" name="features[]" class="form-control d-inline-block w-75" required>
        <button type="button" class="btn btn-danger btn-sm remove-feature">X</button>
    `;
